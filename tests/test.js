...
describe(testName, function() {
  describe('ascii', function() {
    describe('encode', function() {
      it('should be successful', function() {
        for(var i = 0;i < strs.length;++i) {
          expect(B64.encode(strs[i])).to.be(base64Strs[i]);
        }
        expect(B64.encode(strs[0], true)).to.be(base64Strs[0]);
      });
    });
    ...
    describe('decode', function() {
      it('should be successful', function() {
        for(var i = 0;i < strs.length;++i) {
          expect(B64.decode(base64Strs[i])).to.be(strs[i]);
        }
        expect(B64.decode(base64Strs[0], true)).to.be(strs[0]);
      });
    });
  });
  ...
  describe('UTF8', function() {
    describe('encode', function() {
      it('should be successful', function() {
        for(var i = 0;i < utf8Str.length;++i) {
          expect(B64.encode(utf8Str[i])).to.be(base64Utf8Strs[i]);
        }
      });
    });
    ...
    describe('decode', function() {
      it('should be successful', function() {
        for(var i = 0;i < utf8Str.length;++i) {
          expect(B64.decode(base64Utf8Strs[i])).to.be(utf8Str[i]);
        }
      });
    });
  });
